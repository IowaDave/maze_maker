...
let startNewGame = false; // at startup 
...
function displayCell(row: number, col: number) {
    basic.clearScreen();
    if (isEntrance(row, col)) drawPointsToRight();
    if (isExit(row, col)) drawPointsToLeft();
...
/*****************************************************************
 * obtain user inputs for desired numbers of rows and columns
 * for the next maze. The setters are called from the main loop.
 *****************************************************************/
...
/***************************************************************
 * create a new maze having mazeROWS rows and mazeCOLS columns 
 ***************************************************************/
...
function makeMaze() {
    ...
    let unVisited = mazeROWS*mazeCOLS; // 2 x 2 is the startup default
    /********************************************************************************* 
     ...
     *********************************************************************************/
    maze = pins.createBuffer(unVisited); // initializes bytes to 0
    /*********************************************************************************
    ...
    ****************************************************/
       ...
       *********************************************************/
          ...
          maze.getNumber(NumberFormat.Int8LE, index((mazeROWS - 1), col)) + BOTTOMLINE);
  	}
       ...
       **********************************************************/
        maze.setNumber(NumberFormat.Int8LE, index(row, (mazeCOLS - 1)),
          maze.getNumber(NumberFormat.Int8LE, index(row, (mazeCOLS - 1))) + RIGHTLINE);
  	}
    /******************************************************************
    ...
    ******************************************************************/
    ...
    maze.setNumber(NumberFormat.Int8LE, index(origin[0], origin[1]),
       maze.getNumber(NumberFormat.Int8LE, index(origin[0], origin[1])) ^ VISITFLAG);
...
/******************************* */
  	...
  	while (unVisited > 0) {
        ...
        if ((maze.getNumber(NumberFormat.Int8LE, 
            index(destination[0], destination[1])) & VISITFLAG) == 0) {
            ...
            maze.setNumber(NumberFormat.Int8LE,
                ...
                maze.getNumber(NumberFormat.Int8LE,
                    index(destination[0], destination[1])) ^ VISITFLAG);
 ...
 ******************************************************************/
  			...
  			if (motion[0] < 0) {
                // moving up one row, clear top line of origin cell   
                  ...
                  maze.getNumber(NumberFormat.Int8LE, 
                  index(origin[0], origin[1])) ^ TOPLINE);        
  			}
  			if (motion[0] > 0) {
  			...
  			if (motion[1] < 0) {
                ...
                maze.setNumber(NumberFormat.Int8LE, 
                    index(origin[0], origin[1]), 
                    maze.getNumber(NumberFormat.Int8LE, 
                        index(origin[0], origin[1])) ^ LEFTLINE);
  			}
  			if (motion[1] > 0) {
                ...
                maze.setNumber(NumberFormat.Int8LE, 
                    index(destination[0], destination[1]), 
                    maze.getNumber(NumberFormat.Int8LE, 
                        index(destination[0], destination[1])) ^ LEFTLINE);
  			}
